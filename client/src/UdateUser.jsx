...
function UpdateUser() {
  ...
  useEffect( ()=> {
    const fetchUsers =async ()=>{
     try {
       const result = await axios.get("http://localhost:8081/getUser/" +id)
       console.log(result)
   ...
   const handleSubmit = (e)=>{
    ...
    try{
      axios.put("http://localhost:8081/UpdateUser/"+id ,{name,email,phone})
        ...
        <div className='w-50 rounded p-3'>
            ...
            <div class="mb-3">
                 <input type="number" class="form-control"style={{borderColor:"black",borderWidth:"3px"}}
                  placeholder="Number"value={phone} onChange={(e)=>setNumber(e.target.value)}/>
            </div> 
            <div>
